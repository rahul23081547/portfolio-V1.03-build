...
const AlternateRealityAudio: React.FC = () => {
  ...
  // Auto-start alternate reality music if main audio was playing
  useEffect(() => {
    const savedPlaying = localStorage.getItem('audioPlaying');
    if (savedPlaying === 'true' && audioRef.current && !isPlaying) {
        ...
        <div className={`bg-black/90 backdrop-blur-sm p-3 rounded-full shadow-lg border border-gray-600/50 transform transition-all duration-300 ease-in-out ${
          ...
          <div className="flex flex-col items-center h-24">
            <input
              ...
              className="h-20 w-2 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-white slider-vertical"
              style={{
                writingMode: 'bt-lr',
                WebkitAppearance: 'slider-vertical',
                background: `linear-gradient(to top, #ffffff 0%, #ffffff ${volume * 100}%, #374151 ${volume * 100}%, #374151 100%)`
              }}
              orient="vertical"
            />
          </div>
